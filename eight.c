/*
 * Project Euler.
 * Largest product in a series
 * Problem 8
 * 
 * Find the greatest product of five consecutive digits in the 1000-digit number
 */

#include <stdlib.h>
#include <stdint.h>
#include <stdio.h>
#include <string.h>

/*
 * Given a string of integers, convert the single integer at the index from
 * a char to an int
 */
uint32_t get_number( const char *num, const int index )
{
	uint32_t temp_num = 0 ;
	char *endptr ;
	char x[2] ;
	memset( x, 0x0, 2 ) ;
	memcpy( x, &num[index], 1 ) ;
	temp_num = strtol( x, &endptr, 10 ) ;

	return temp_num ;
}

int main( int argc, char * argv[] )
{
    uint32_t sum = 0 ;
    uint32_t temp_sum = 0 ;
    uint32_t i = 4 ;
    uint32_t len = 1000 ;

    const char * number = "" ;

    while ( i < len )
    {
    	// if the number is a 0 or 1, then the new product won't be bigger than the previous product.
        if ( (number[i] == '1') || (number[i] == '0') )
        {
            i += 5 ;
        }
        else
        {
			uint32_t a, b, c, d, e ;
			a = get_number( number, i ) ;
			b = get_number( number, i-1 ) ;
			c = get_number( number, i-2 ) ;
			d = get_number( number, i-3 ) ;
			e = get_number( number, i-4 ) ;
            temp_sum = a * b * c * d * e ;
            if ( temp_sum > sum )
            {
                sum = temp_sum ;
            }
            i++ ;
        }
    }

	printf( "%d\n", sum ) ;

    return 0 ;
}
